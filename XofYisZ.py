...
def find_compound(parced, h, x, mustBeNoun):
...
def find_how_xyz_format(parced, h, iteration):
    #When was y x(verb)?
    
    #Find x
    xObject = h.head.head
    x = find_compound(parced, h, xObject, True) #Find anything that might need to be included in x
    if (iteration == 0):
        x = "cause of "+x
    if (iteration == 1):
        x = "manner of "+x
    ...
    for i in parced:
        if (i.head == xObject and i.dep_ == "nsubj"):
            yObject = i
            y = find_compound(parced, h, yObject, True) #Find anything that might need to be included in y
            break
...
def find_when_xyz_format(parced, h):
    #When was y x(verb)?
...
def find_standard_xyz_format(parced, h, propDate):
    #Find x
    xObject = h.head
    x = ""
    if (not xObject.pos_ == "PRON"):
        x = find_compound(parced, h, xObject, False) #Find anything that might need to be included in x
        if(propDate):
            x = "date of" + x
    ...
    for i in parced:
        if (i.head == h and not i.pos_ == "PRON"):
            yObject = i
            y = find_compound(parced, h, yObject, False) #Find anything that might need to be included in y
            break
    ...
    if (possibleIs.lemma_ == "be"):
        for i in parced:
            if ((i.pos_ == "NOUN" or i.pos_ == "PROPN") and i.head == possibleIs and not i == xObject):
                zObject = i
                z = find_compound(parced, h, zObject, False) #Find anything that might need to be included in z
                break
    return [x, y, z]
...
def find_x_of_y_is_z(parced):
    for numh in range(len(parced)-1, -1, -1):
        h = parced[numh]
        print (h.lemma_)
        if (h.dep_ == "prep" and h.lemma_ == "of"):
            li = find_standard_xyz_format(parced, h, False)
            x = li[0]
            y = li[1]
            z = li[2]
        ...
        if (h.head.pos_ == "VERB" and h.lemma_ == "when"):
            ...
            if (h.head.dep_ == "ROOT"):
                ...
                li = find_standard_xyz_format(parced, h, True)
                x = li[0]
                y = li[1]
                z = li[2]
            ...
            li = find_when_xyz_format(parced, h)
            x = li[0]
            y = li[1]
            z = li[2]
    ...
    if (h.head.pos_ == "VERB" and h.lemma_ == "how"):
        for i in range(0, 1, 2):
            li = find_how_xyz_format(parced, h, i)
            x = li[0]
            y = li[1]
            z = li[2]
